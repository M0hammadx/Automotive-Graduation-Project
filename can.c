        ...
        if(CANSTS & 0x80) {  // busoff
         ...
         //transmit
		  if (0 < objectnum && objectnum <= Max_transmitObjects){
               ...
               CanIf_TxConfirmation(TrnsmPDU[Controller][CANINT].swPduHandle);
                
            }
			objectnum = HW_READ_REG_32BIT(BaseAddress[Controller]+CAN_CANINT_OFFSET);
						//receive
		  if(objectnum > Max_transmitObjects && objectnum <= Max_ReceiveObjects+Max_transmitObjects){
            
               HW_RESET_BIT(BaseAddress[Controller]+CAN_CANSTS_OFFSET,4);   // RXOK  
               HW_WRITE_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2CMSK_OFFSET,0x98,0xff);  //clearpnd
               CANINT =  HW_READ_REG_32BIT(BaseAddress[Controller]+CAN_CANINT_OFFSET);     
               HW_WRITE_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2CRQ_OFFSET,CANINT,0x3f);
                 
              HW_WRITE_REG_32BIT(BaseAddress[Controller]+CAN_CANIF2CMSK_OFFSET,0x73);
              HW_WRITE_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2CRQ_OFFSET,CANINT,0x3f);
              dataReceived[0] = HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DA1_OFFSET,0x00FF);
              dataReceived[1] = (HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DA1_OFFSET,0xFF00)>>8);
              dataReceived[2] = (HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DA2_OFFSET,0x00ff));
              dataReceived[3] = (HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DA2_OFFSET,0xFF00)>>8);
              dataReceived[4] = (HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DB1_OFFSET,0x00ff));
              dataReceived[5] = ((HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DB1_OFFSET,0xFF00))>>8);
              dataReceived[6] = (HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DB2_OFFSET,0x00ff));
              dataReceived[7] = ((HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2DB2_OFFSET,0xFF00))>>8);
              DLC = HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2MCTL_OFFSET,0x0f);
            
             Can_Receive.CanId = HW_READ_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF2ARB2_OFFSET,0x1ffc);
...
void Can_Init( const Can_ConfigType* Config ){
    ...
    for(i =0 ; i<ControllerNum ; i++ ){
       ...
       /* HW_SET_BIT(CanBase+CAN_CANCTL_OFFSET,7);
        ...
        Can_SetBaudrate(CanControllerobj.CanControllerId,CanControllerobj.CanControllerBaudRateConfigobj->CanControllerBaudRateConfigID);
       
        installNIVCISRFunction(CanControllerobj.CanControllerId+39, &Can_InterruptHandler ); ///////////////////////////////////////////////////
        NVICInterruptEnable(39+i,0,0);

        for(j=0 ; j<MAX_MSG_NUM ;j++){
            
             if( CanBase == Config->HOH[j].CanControllerRef->CanControllerBaseAddress){
                
                 
                 //initialize transmitted messages 
                 if(Config->HOH[j].CanObjectType == 0){
                    Max_transmitObjects ++;
                    HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1CMSK_OFFSET,VALUEIF1CMSK,0xff);
                    if(Config->HOH[j].CanIdType == STANDARD){            // 11 bit
                        
                        mask = ((Config->HOH[j].CanHwFilterptr->CanHwFilterMask)<<2);
                        ...
                        ID=((Config->HOH[j].CanHwFilterptr->CanHwFilterCode)<<2)|(1<<13)|(1<<15);  // ID and msgval
                        HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1ARB2_OFFSET,ID,0xffff);
                    }   
                    else if (Config->HOH[j].CanIdType == EXTENDED){
                        mask =(Config->HOH[j].CanHwFilterptr->CanHwFilterMask)  ;  //29 bit
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1MSK1_OFFSET,mask&0x0000ffff);
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1MSK2_OFFSET,((mask&0x1fff0000)>>16));
                        ...
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1ARB1_OFFSET,ID&0x0000FFFF);
                        HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1ARB2_OFFSET,(ID&0x1FFF0000>>16),0x1FFF);
                    ...
                    HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1MCTL_OFFSET,CANIF1MCTL);        //TxIE-UMASK-EOB-DLC=8
                    msgNumb = Config->HOH[j].CanObjectId;
                    HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1CRQ_OFFSET,msgNumb,0x3f);        // message number        
                }
                //initialize received messages
                else if(Config->HOH[j].CanObjectType == 1){
                    Max_ReceiveObjects ++ ;
                    HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF2CMSK_OFFSET,VALUEIF2CMSK,0xff);
                   if(Config->HOH[j].CanIdType == STANDARD){            // 11 bit
                        mask = ((Config->HOH[j].CanHwFilterptr->CanHwFilterMask)<<2 );
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF2MSK2_OFFSET,mask);
                        HW_SET_BIT(CanBase+CAN_CANIF2MSK2_OFFSET,14);            //MDIR
                        ID=((Config->HOH[j].CanHwFilterptr->CanHwFilterCode)<<2)|(1<<15);  // ID and msgval
                        HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF2ARB2_OFFSET,ID,0xffff);
                   }
                   else if (Config->HOH[j].CanIdType == EXTENDED){
                        mask =(Config->HOH[j].CanHwFilterptr->CanHwFilterMask)  ;  //29 bit
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF2MSK1_OFFSET,mask&0x0000ffff);

                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF2MSK2_OFFSET,((mask&0x1fff0000)>>16));
                        HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF2MSK2_OFFSET,0xC000,0xC000) ;  //MXTD-MDIR
                        ID= Config->HOH[j].CanHwFilterptr->CanHwFilterCode  ;  // ID 
                        HW_WRITE_REG_32BIT(CanBase+CAN_CANIF2ARB1_OFFSET,ID&0x0000FFFF);
                        HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF2ARB2_OFFSET,(ID&0x1FFF0000>>16),0x1FFF);
                    ...
                    HW_WRITE_REG_32BIT(CanBase+CAN_CANIF2MCTL_OFFSET,CANIF2MCTL);        //RxIE-UMASK-EOB-DLC=8
                    msgNumb = Config->HOH[j].CanObjectId;
                    HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF2CRQ_OFFSET,msgNumb,0x3f);        // message number        
               }
...
Std_ReturnType Can_SetBaudrate( uint8 Controller, uint16 BaudRateConfigID  ){
    ...
    uint32 x;
    if (ControllerMode[Controller] != CAN_CS_STOPPED) return E_NOT_OK ;
   
    for(j=0 ; j< ControllerNum ;j++){
       
        if( GlobalConfig->canControllers[j].CanControllerId == Controller){
            HW_SET_BIT(GlobalConfig->canControllers[j].CanControllerBaseAddress+CAN_CANCTL_OFFSET,6);    // set CCE bit to write in CANBIT register
                
            if(GlobalConfig->canControllers[j].CanControllerBaudRateConfigobj->CanControllerBaudRateConfigID==BaudRateConfigID){
                    BaudRateConfig = (GlobalConfig->canControllers[j].CanControllerBaudRateConfigobj);
				           	n = BaudRateConfig->CanControllerSeg2 +BaudRateConfig->CanControllerSeg1 +BaudRateConfig->CanControllerPropSeg +1;
					          BRP = (CAN_CLOCK / ((BaudRateConfig->CanControllerBaudRate)*1000 *n))-1;
					          TSEG2 =(BaudRateConfig->CanControllerSeg2) - 1 ;       //Time Segment after Sample Point
					          TSEG1 =(( BaudRateConfig->CanControllerSeg1)+ BaudRateConfig->CanControllerPropSeg )- 1 ; 
				          	SJW   = (BaudRateConfig->CanControllerSyncJumpWidth) - 1 ;   //Synchronization Jump Width
                     x    = ((TSEG2<<12)&0x7000)|((TSEG1<<8)&0xf00)|((SJW<<6)&0xC0)|BRP;  
                     HW_WRITE_REG32_MASKED(GlobalConfig->canControllers[i].CanControllerBaseAddress+CAN_CANBIT_OFFSET,x,0x3FFF);
                     HW_RESET_BIT(GlobalConfig->canControllers[j].CanControllerBaseAddress+CAN_CANCTL_OFFSET,6);     // reset CCE
                     break;
...
Can_ReturnType Can_SetControllerMode( uint8 Controller, Can_StateTransitionType Transition ){
    uint32 CanBase ;
    if((Transition == CAN_CS_STARTED && (ControllerMode[Controller] != CAN_CS_STOPPED)) ||
        (Transition == CAN_CS_STOPPED && (ControllerMode[Controller] != CAN_CS_SLEEP || ControllerMode[Controller] != CAN_CS_STARTED))||
         (Transition == CAN_CS_SLEEP && ControllerMode[Controller] != CAN_CS_STOPPED))
        {
            return E_NOT_OK;
        }
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo ){
    ...
    for(j=0 ; j< MAX_MSG_NUM ;  j++){
        if (GlobalConfig->HOH[j].CanObjectType == TRANSMIT && GlobalConfig->HOH[j].CanObjectId == Hth ){
            CanBase = GlobalConfig->HOH[j].CanControllerRef->CanControllerBaseAddress ; 
    ...
    if(HthValid== false || PduInfo == NULL) return CAN_NOT_OK ;
    
    DLC = PduInfo->length;
    if (!(HW_READ_BIT(CanBase+CAN_CANTXRQ1_OFFSET,Hth))){               // if The message object is not waiting for transmission
        TrnsmPDU[GlobalConfig->HOH[j].CanControllerRef->CanControllerId][Hth] = *PduInfo ;
       
        if(PduInfo->id == msgObj.CanHwFilterptr->CanHwFilterCode){
            if(DLC <= 4){ 
                HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1CMSK_OFFSET,0x92,0xff);    //WRNR-CONTROL-DATAA-  
            }else if(DLC >4 && DLC<=8) {
                HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1CMSK_OFFSET,0x93,0xff);    //WRNR-CONTROL-DATAA-DATAB
            }
            HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1MCTL_OFFSET,(0x1080|DLC),0x0F);        // DLC 
             
            HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1DA1_OFFSET,((PduInfo->sdu[1]<<8)+(PduInfo->sdu[0])));
            HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1DA2_OFFSET,((PduInfo->sdu[3]<<8)+(PduInfo->sdu[2])));
            HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1DB1_OFFSET,((PduInfo->sdu[5]<<8)+(PduInfo->sdu[4])));
            HW_WRITE_REG_32BIT(CanBase+CAN_CANIF1DB2_OFFSET,((PduInfo->sdu[7]<<8)+(PduInfo->sdu[6])));
            
           if(msgObj.CanTriggerTransmitEnable == TRUE){ 
                HW_SET_BIT(CanBase+CAN_CANIF1CMSK_OFFSET,2);             //TXRQT
            }
            if(HW_READ_BIT(CanBase+CAN_CANIF1CRQ_OFFSET,15)){}
            HW_WRITE_REG32_MASKED(CanBase+CAN_CANIF1CRQ_OFFSET,Hth,0x3f);        // message number
            
        }
         return CAN_OK ;
    }else return CAN_BUSY;
...
void Can_MainFunction_BusOff( void ){
   //fe hna moshkla lw kr2t el sts register
    for(i=0 ; i< ControllerNum ;i++){
     uint32 cansts = HW_READ_BIT(GlobalConfig->canControllers[i].CanControllerBaseAddress+CAN_CANSTS_OFFSET,7);
     if(cansts ==1) {
        ControllerMode[GlobalConfig->canControllers[i].CanControllerId] = CAN_CS_STOPPED;
