...
sSysCtrlError_t clockGating(sSysCtrlModules_t obj) {
	...
	if (obj == GPIOA) {
		...
		/*Enable UART7 clock. */
                ...
                HW_WRITE_BIT(
						CAN_0 + CAN_RUN_MODE_CLOCK_GATING_CONTROL_OFFSET,
						0, HIGH);
    }else if(obj == CAN_1){
                HW_WRITE_BIT(
						CAN_1 + CAN_RUN_MODE_CLOCK_GATING_CONTROL_OFFSET,
						1, HIGH);
    }else {
...
sSysCtrlError_t NVICInterruptEnable(g_ISRVirIdx_t obj, uint8 priority,
	...
	if (obj == GPIOA_IRQ) {
        ...
        /* enable interrupt*/
		HW_WRITE_BIT(INTERRUPT_32_63_SET_ENABLE, CAN0_INT_NUMBER-32, HIGH);
		/* assign priority to PRI9 register*/
		HW_WRITE_REG32_MASKED(INTERRUPT_36_39_PRIORITY, priority << 29,0X7UL << 29);
		/* set interrupt pending*/
		HW_WRITE_BIT(INTERRUPT_32_63_SET_PENDING, CAN0_INT_NUMBER-32, pending);
    }else if (obj == CAN1){
        /* enable interrupt*/
		HW_WRITE_BIT(INTERRUPT_32_63_SET_ENABLE, CAN1_INT_NUMBER-32, HIGH);
		/* assign priority to PRI9 register*/
		HW_WRITE_REG32_MASKED(INTERRUPT_40_43_PRIORITY, priority << 5,0X7UL << 5);
		/* set interrupt pending*/
		HW_WRITE_BIT(INTERRUPT_32_63_SET_PENDING, CAN1_INT_NUMBER-32, pending);
    }else {
		state = IRQ_NOT_EXIST;
	}
	return state;
 ...
 // reset clock
sSysCtrlError_t ResetClockGating(sSysCtrlModules_t obj) {
	sSysCtrlError_t state = PASSED;
   
    if(obj == CAN_0){
                HW_WRITE_BIT(
						BASE_SYSTEM_CONTROL + CAN_RUN_MODE_CLOCK_GATING_CONTROL_OFFSET,
						0,LOW);
    }else if(obj == CAN_1){
                HW_WRITE_BIT(
						BASE_SYSTEM_CONTROL + CAN_RUN_MODE_CLOCK_GATING_CONTROL_OFFSET,
