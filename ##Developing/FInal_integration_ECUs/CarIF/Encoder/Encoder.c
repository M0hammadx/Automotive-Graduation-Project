...
void Encoder_Get_Distance_Elapsed(double *Ptrdistance)
{
...
void Encoder_Get_Motors_RPM(uint16_t* RPM)
{
    ...
    RPM[0] = (system_freq * 60 * Encoder0_read)
            / (Encoder_timer_Load * gear_ratio * Max_Position); // calculate RPM for motor 0
    RPM[1] = (system_freq * 60 * Encoder1_read)
            / (Encoder_timer_Load * gear_ratio * Max_Position); // calculate RPM for motor 1
