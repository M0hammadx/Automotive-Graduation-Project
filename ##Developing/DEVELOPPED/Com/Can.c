...
uint32 errorAndWarnning =ERROR_AND_WRNING ;
//static uint32_t E=0;
...
//            System_printf("TxOK !\n");
//            System_flush();
            errorAndWarnning = ERROR_AND_WRNING;
        }
        if(CANSTS & 0x10){
            HW_RESET_BIT(BaseAddress[Controller]+CAN_CANSTS_OFFSET,4);
//            System_printf("RxOK !\n");
//            System_flush();
            UART_OutString("RxOK !\n");
            errorAndWarnning = ERROR_AND_WRNING;
        ...
        if(CANSTS & 0x20){
//            E++;
            UART_OutString("Error Passive !\n %d");
            HW_RESET_BIT(BaseAddress[Controller]+CAN_CANSTS_OFFSET,5);
//            System_flush();
            errorAndWarnning--;
        }
        if(CANSTS & 0x40){
            UART_OutString("Error warnning !\n");
//            System_flush();
            errorAndWarnning--;
        }
//        if(errorAndWarnning == 0){
//            System_printf("Explicit Buss Off ####################################!\n");
//            System_flush();
//            errorAndWarnning = ERROR_AND_WRNING;
...
//                HW_WRITE_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF1CRQ_OFFSET,CANINT,0x3f);
            HW_WRITE_REG32_MASKED(BaseAddress[Controller]+CAN_CANIF1CRQ_OFFSET,
                                  objectnum, 0x3f);
            HW_RESET_BIT(BaseAddress[Controller]+CAN_CANSTS_OFFSET,3);
 //////////////////////////////////////////
uint32 pnd = HW_READ_REG_32BIT(
          BaseAddress[Controller]+CAN_CANMSG1INT_OFFSET);
  uint32 pnd2 = HW_READ_REG_32BIT(
             BaseAddress[Controller]+CAN_CANMSG2INT_OFFSET);
  //uint8 objType ;
  if (pnd2 || pnd){
      q++;
  }
...
void Can_EnableControllerInterrupts(uint8 Controller)
{
    ...
    if (Can_DisableInterruptCount[Controller] == 1)
    {
       ...
       // HW_RESET_BIT(BaseAddress[Controller]+CAN_CANSTS_OFFSET,2);
                                     ...
                                     Can_StateTransitionType Transition)
{
    ...
    else if (Transition == CAN_CS_STOPPED)
    {
        ControllerMode[Controller] = CAN_CS_STOPPED;
        HW_SET_BIT(CanBase+CAN_CANCTL_OFFSET, 0); // set init bit to enter init mode
        //HW_RESET_BIT(CanBase+CAN_CANCTL_OFFSET,1); //disable interrupt
...
Can_ReturnType Can_Write(Can_HwHandleType Hth, const Can_PduType* PduInfo)
{
    ...
    DLC = PduInfo->length;
    if (!(HW_READ_BIT(CanBase + CAN_CANTXRQ1_OFFSET, Hth-1)))
