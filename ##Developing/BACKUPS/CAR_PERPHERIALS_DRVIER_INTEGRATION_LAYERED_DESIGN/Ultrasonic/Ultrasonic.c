...
void Ultrasonic_Init(){
  ...
  while(ROM_SysCtlPeripheralReady(SYSCTL_PERIPH_TIMER0)!=true);
  ...
  //WT 2 
	...
	while(ROM_SysCtlPeripheralReady(SYSCTL_PERIPH_WTIMER2)!=true);
			...
			//portA trigger
			//A4,5,6,7			
	ROM_GPIODirModeSet(GPIO_PORTA_BASE, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7
                           ,GPIO_DIR_MODE_OUT);		
	ROM_GPIOPadConfigSet(GPIO_PORTA_BASE, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7
...
//Left   Echo->PD1   Back Trigh->PA6    Front  Trig->PA7
...
//Left_Back     Echo->PF2    Trigh->PA6  
//Left_Front    Echo->pF3    Trig->PA7
...
if(Front_Left_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_0, 0x01);}
if(Front_Right_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_1, 0x02);}
if(Right_Front_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_2, 0x04);}
if(Right_Back_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_3, 0x08);}
if(Back_Right_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_4, 0x10);}
if(Back_Left_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_5, 0x20);}
if(Left_Back_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_6, 0x40);}
if(Left_Front_Ultrasonic_Trig){ROM_GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_7, 0x80);}
...
ROM_GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_4 |GPIO_PIN_5 |GPIO_PIN_6 |GPIO_PIN_7, 0x00);
...
if((ultrasonic & Front_Left_Ultrasonic)&&(Front_Left_Ultrasonic_flag ==0)&&(Front_Right_Ultrasonic_flag ==0)){
Front_Left_Ultrasonic_flag =1;
Front_Left_Ultrasonic_Trig =1;	
ROM_TimerEnable(TIMER2_BASE,TIMER_A);
TimerIntRegister (TIMER2_BASE, TIMER_A,Front_Left_Ultrasonic_Handler);}

 if((ultrasonic &Front_Right_Ultrasonic)&&(Front_Left_Ultrasonic_flag ==0)&&(Front_Right_Ultrasonic_flag ==0)){
Front_Right_Ultrasonic_flag = 1 ;
Front_Right_Ultrasonic_Trig = 1 ;	 
ROM_TimerEnable(TIMER2_BASE,TIMER_A);
...
if((ultrasonic &Right_Front_Ultrasonic)&&(Right_Front_Ultrasonic_flag ==0)&&(Right_Back_Ultrasonic_flag ==0)){
Right_Front_Ultrasonic_flag = 1 ;
Right_Front_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(TIMER3_BASE,TIMER_A);
TimerIntRegister (TIMER3_BASE, TIMER_A,Right_Front_Ultrasonic_Handler);}

if((ultrasonic &Right_Back_Ultrasonic)&&(Right_Front_Ultrasonic_flag ==0)&&(Right_Back_Ultrasonic_flag ==0)){
Right_Back_Ultrasonic_flag = 1 ;
Right_Back_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(TIMER3_BASE,TIMER_A);
TimerIntRegister (TIMER3_BASE, TIMER_A,Right_Back_Ultrasonic_Handler);}

if((ultrasonic &Back_Right_Ultrasonic)&&(Back_Right_Ultrasonic_flag ==0)&&(Back_Left_Ultrasonic_flag ==0)){
Back_Right_Ultrasonic_flag = 1 ;
Back_Right_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(TIMER3_BASE,TIMER_B);
TimerIntRegister (TIMER3_BASE, TIMER_B,Back_Right_Ultrasonic_Handler);}

if((ultrasonic &Back_Left_Ultrasonic)&&(Back_Right_Ultrasonic_flag ==0)&&(Back_Left_Ultrasonic_flag ==0)){
Back_Left_Ultrasonic_flag = 1 ;
Back_Left_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(TIMER3_BASE,TIMER_B);
TimerIntRegister (TIMER3_BASE, TIMER_B,Back_Left_Ultrasonic_Handler);}

if((ultrasonic &Left_Back_Ultrasonic)&&(Left_Back_Ultrasonic_flag ==0)&&(Left_Front_Ultrasonic_flag ==0)){
Left_Back_Ultrasonic_flag = 1 ;
Left_Back_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(WTIMER2_BASE,TIMER_B);
TimerIntRegister (WTIMER2_BASE, TIMER_B,Left_Back_Ultrasonic_Handler);}

if((ultrasonic &Left_Front_Ultrasonic)&&(Left_Back_Ultrasonic_flag ==0)&&(Left_Front_Ultrasonic_flag ==0)){
Left_Front_Ultrasonic_flag = 1 ;
Left_Front_Ultrasonic_Trig = 1 ;	
ROM_TimerEnable(WTIMER2_BASE,TIMER_B);
...
if (s!=ULTRASONIC) {Ultrasonic_Read_Routine(ULTRASONIC);}
}	
}
...
uint32_t Ultrasonic_Get_Distance( uint8_t distant){
	static int devisor;
	if(distant < 8){	
  ...
  ROM_TimerIntClear(TIMER3_BASE, TIMER_CAPA_EVENT);  
    ...
    {   
        low = ROM_TimerValueGet(TIMER3_BASE, TIMER_A);
			Ultrasonic_Reading[Righ_Front] = (low - high);
 ...
 void delay_microSec(int time)
{
  ...
  ROM_TimerLoadSet(TIMER0_BASE, TIMER_A,SysCtlClockGet()/1000000);   
	ROM_TimerEnable(TIMER0_BASE,TIMER_A);
     
    for ( i = 0; i < time; i++)
    {
        while (ROM_TimerIntStatus(TIMER0_BASE,false) == 0);
        ROM_TimerIntClear(TIMER0_BASE,TIMER_TIMA_TIMEOUT);
    }
    ROM_TimerDisable(TIMER0_BASE, TIMER_A);    // disable timer
}
/*
