...
/////////////////***************************************
 typedef struct  CanControllerFdBaudrateConfigc{
    ...
    bool    CanControllerTxBitRateSwitch ;              //default false 
...
typedef struct CanGeneralc{
    ...
    bool   CanDevErrorDetect ;                //Switches the development error detection and notification on or off
    bool   CanMultiplexedTransmission ;            //Specifies if multiplexed transmission shall be supported.ON or OFF
    ...
    bool   CanVersionInfoApi  ;                   //Switches the Can_GetVersionInfo() API ON or OFF.
    bool   CanPublicIcomSupport ;                 //Selects support of Pretended Network features in Can driver..true/false
    ...
    */
    ...
    bool CanSetBaudrateApi    ;                     //If this parameter is set to true the Can_SetBaudrate API shall be supported. Otherwise the API is not supported.
...
typedef struct CanIcomRxMessagec{
        ...
        bool    CanIcomPayloadLengthError ;        //This parameter defines that the MCU shall wake if a payload error occurs
    /*
...
typedef struct CanIcomc{
    struct CanIcomConfigc{
        ...
        bool   CanIcomWakeOnBusOff ;        // default true    
...
typedef struct CanControllerc{
    ...
    bool CanControllerActivation   ;              //Defines if a CAN controller is used in the configuration.
    uint32 CanControllerBaseAddress ;            //Specifies the CAN controller base address.
    bool  CanWakeupSupport  ;                     //CAN driver support for wakeup over CAN Bus.
    bool CanWakeupFunctionalityAPI  ;            // Adds / removes the service Can_CheckWakeup() from the code.True: Can_CheckWakeup can be used. False: Can_CheckWakeup cannot be used.
...
typedef struct CanConfigType{
    ...
    CanController canControllers[ControllerNum] ;
   
    CanHardwareObject HOH[MAX_MSG_NUM] ;
