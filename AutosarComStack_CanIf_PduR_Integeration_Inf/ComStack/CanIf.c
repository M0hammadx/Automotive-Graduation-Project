...
#endif
{
	if (TxPduId >= CanIf_ConfigPtr->CanIfInitCfg->CanIfMaxTxPduCfg) {
		...
		return  (CanIfTxPduCfgType *const)(&CanIf_ConfigPtr->CanIfInitCfg->CanIfTxPduCfg[TxPduId]);
	}
}
...
Std_ReturnType CanIf_Transmit( PduIdType TxPduId, const PduInfoType* PduInfoPtr )
{
    ...
    */
    ...
    Std_ReturnType retVal = Can_Write(TxPduCfgPtr->CanIfTxPduBufferRef->CanIfBufferHthRef->CanIfHthIdSymRef->CanObjectId, &canPdu);
    if (retVal == E_NOT_OK){
        ///Report error
        return E_NOT_OK;
