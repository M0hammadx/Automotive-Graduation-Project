...
#define SSID_NAME  "Mohab"
#define PASSKEY    "0100550905"
...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
  ...
  GPIO_PORTE_AFSEL_R |= 0x30; //E4,E5   E4 Rx    E5 Tx
  GPIO_PORTE_DIR_R |= 0x08; // PE3 output to reset
  GPIO_PORTE_PCTL_R =(GPIO_PORTE_PCTL_R&0xFF000FFF)|0x00110000;
  GPIO_PORTE_DEN_R   |= 0x38; //E3 E4 E5 
  GPIO_PORTE_DATA_R |= 0x08; // reset high
...
int ESP8266_Reset(){int try=MAXTRY;
  SearchStart("ready");
  while(try){
    GPIO_PORTE_DATA_R &= ~0x08; // reset low
    DelayMs(10);
    GPIO_PORTE_DATA_R |= 0x08; // reset high
